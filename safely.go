...
//
// commonlib.Go handles this by trapping the panic and writing it to the default
// logger.
//
	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				...
				log.Printf("Panic in commonlib.Go: %s", err)
			}
		}()
		todo()
	}()
...
func GoDo(cxt Logger, todo GoDoer) {
	go func() {
		defer func() {
			if err := recover(); err != nil {
				cxt.Logf("error", "Panic in commonlib.Go: %s", err)
			}
		}()
		todo()
	}()
...
func GoLog(logger SafelyLogger, todo GoDoer) {
	go func() {
		defer func() {
			if err := recover(); err != nil {
				logger.Printf("Panic in commonlib.Go: %s", err)
			}
		}()
		todo()
	}()
